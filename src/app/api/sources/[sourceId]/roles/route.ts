...
import { resolveSourceRoles } from "@/lib/source-catalog-resolver"
...
export async function GET(
  ...
  try {
    const { items, catalogSource } = await resolveSourceRoles(apiKey, sourceId)
    return NextResponse.json({ roles: items, catalogSource })
  } catch (err) {
    ...
    return NextResponse.json(
      { error: logAndSafeError("sources/roles", err, "Failed to list roles") },
      { status: 502 },
    )
  }
}
