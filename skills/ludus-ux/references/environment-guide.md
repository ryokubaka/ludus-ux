> Adapted summary from [badsectorlabs/ludus-skills](https://gitlab.com/badsectorlabs/ludus-skills).  
> **Full upstream:** `skills/ludus/environment-guide/SKILL.md` + `skills/ludus/environment-guide/references/environments.md`  
> **GOAD via LUX:** prefer `skills/ludus-ux/references/workflows/goad-deploy.md` (not inventing range YAML).
...
### GOAD (Game of Active Directory) — use LUX, do not invent YAML

**Description:** Multi-VM Active Directory labs (GOAD, **GOAD-Mini**, NHA, SCCM, …) from Orange Cyberdefense’s GOAD project. In **Ludus UX**, these are deployed via the **GOAD** UI/API — not by inventing a one-VM `config.yml`.

**LUX (preferred):**
1. Open **`/goad`** → Deploy New Instance (or `getGoadCatalog` / `executeGoad`).
2. Pick lab type from the live catalog (e.g. GOAD-Mini). Optionally add extensions.
3. LUX creates/links a Ludus range and runs GOAD Provide/Install + Ludus deploy.

**Ludus blueprint (alternate):** Install the **`goad`** blueprint from Sources/Blueprints (when available), then deploy that YAML via New Range → From Blueprint. Ask the user which path if they only say “config.yml for GOAD-mini”.

**Do not:** invent `goad-mini-template` or a fake single-host range config. Read `skills/ludus-ux/references/goad.md` and `docs/workflows.md`.

**Templates often needed (verify with `listTemplates`):** Windows Server templates such as `win2019-server-x64-template` / `win2016-server-x64-template` (exact names from the catalog/lab).

**Ref:** https://github.com/Orange-Cyberdefense/GOAD · LUX docs `docs/workflows.md`

---

### GOAD-NHA / GOAD-SCCM / GOAD-DRACARYS

Same rule: prefer LUX GOAD catalog lab ids when present. DRACARYS and challenge variants may need extra templates — look up docs; do not invent YAML.
