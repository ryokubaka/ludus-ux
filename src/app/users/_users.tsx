  ...
  const startImpersonate = useCallback(async (user: UserObject) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({
          variant: "destructive",
          title: "Could not auto-fetch API key",
      ...
      if (data.message) {
        toast({
          variant: "destructive",
          title: "Could not auto-read ~/.bashrc",
                          ...
                          <div className="flex items-center justify-end gap-1">
                            ...
                            >
                              {roleChanging === user.userID
                                ? <Loader2 className="h-3 w-3 animate-spin" />
                                : user.isAdmin
                                  ? <ShieldOff className="h-3 w-3 text-status-warning" />
                            ...
                            <Button size="icon-sm" variant="ghost"
                              ...
                              {apiKeyLoading === user.userID
                                ? <Loader2 className="h-3 w-3 animate-spin" />
                                : <Key className="h-3 w-3 text-status-warning" />}
                            </Button>
                            <Button size="icon-sm" variant="ghost"
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-7 gap-1.5 border-status-info/40 text-status-info hover:bg-status-info/10 text-xs whitespace-nowrap"
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="users-add-id">User ID <span className="text-status-error">*</span></Label>
              <Input
                id="users-add-id"
                placeholder="jd"
                value={newUserId}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="users-add-password">Password <span className="text-status-error">*</span></Label>
              <div className="flex gap-2">
                <Input id="users-add-password" type={showNewPassword ? "text" : "password"}
            ...
            {(confirmDelete?.rangeIds ?? []).length > 0 && (
              <div className="flex items-start gap-3 p-3 rounded-lg border border-border bg-muted/30">
                ...
                <label htmlFor="delete-range" className="text-xs cursor-pointer">
                  <span className="font-medium text-status-aborted">Also delete range{(confirmDelete?.rangeIds?.length ?? 0) > 1 ? "s" : ""}: </span>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              {apiKeyResult.bashrcUpdated ? (
                <Alert>
                  <AlertDescription className="text-xs text-status-success">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    aria-label={showImpersonateKey ? "Hide impersonation API key" : "Show impersonation API key"}
                    onClick={() => setShowImpersonateKey(!showImpersonateKey)}
                  >
      ...
      <Dialog open={!!changePwUserId} onOpenChange={() => setChangePwUserId(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="users-change-password">New Password <span className="text-status-error">*</span></Label>
              <div className="flex gap-2">
                <Input id="users-change-password" type={showChangePw ? "text" : "password"}
                ...
                <Button
                  ...
                  size="icon"
                  variant="ghost"
                  aria-label={showChangePw ? "Hide new password" : "Show new password"}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="users-change-password-confirm">Confirm Password <span className="text-status-error">*</span></Label>
              <Input id="users-change-password-confirm" type={showChangePw ? "text" : "password"}
                value={changePwConfirm} onChange={(e) => setChangePwConfirm(e.target.value)}
                placeholder="Confirm new password"
                className={cn(changePwConfirm && changePwValue !== changePwConfirm ? "border-status-error" : "")} />
