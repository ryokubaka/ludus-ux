...
function ImpersonationBanner() {
  const { impersonation, exitImpersonation, exitingImpersonation } = useImpersonation()
  if (!impersonation && !exitingImpersonation) return null
  ...
  return (
    <div className="flex items-center gap-3 bg-status-warning/10 border-b border-status-warning/30 px-4 py-2.5">
      ...
      >
        ...
        {exitingImpersonation ? "Returning…" : "Exit Impersonation Mode"}
      </button>
...
/** Full-screen cover while leaving impersonation — hides GOAD/error flash. */
function ExitingImpersonationOverlay() {
  const { exitingImpersonation } = useImpersonation()
  if (!exitingImpersonation) return null
  return (
    <div
      className="fixed inset-0 z-[200] flex flex-col items-center justify-center gap-3 bg-background/90 backdrop-blur-sm"
      role="status"
      aria-live="polite"
    >
      <Loader2 className="h-8 w-8 animate-spin text-status-warning" />
      <p className="text-sm font-medium text-foreground">Returning to your account…</p>
  ...
  return (
    <div className="flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden bg-background">
      ...
      >
        ...
        <div
          className={cn(
            "flex flex-1 flex-col min-h-0 overflow-x-hidden overflow-y-auto p-6",
            exitingImpersonation && "invisible pointer-events-none",
          )}
          aria-hidden={exitingImpersonation || undefined}
        >
          {children}
        </div>
      </main>
      <ExitingImpersonationOverlay />
    </div>
  )
}
